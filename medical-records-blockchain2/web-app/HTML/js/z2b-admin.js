...
function listAssets()
{
    ...
    $.when($.post('/fabric/admin/getAssets', options)).done(function (_results)
    {
        ...
        if (_results.result === 'success')
        {
            _str += '<table width="100%"><tr><th>Request Number</th><th>Status</th><th>Requestor/Owner</th></tr>';
            for (let each in _results.requests)
            {(function(_idx, _arr){
                _str += '<tr><td align="center">'+_arr[_idx].requestNumber+'</td><td>'+JSON.parse(_arr[_idx].status).text+'</td><td>'+_arr[_idx].requestorId+'<br/>'+_arr[_idx].ownerId+'</td></tr>';
            })(each, _results.requests);}
            _str += '</ul>';
